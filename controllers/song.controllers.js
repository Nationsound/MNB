...
function uploadBufferToCloudinary(buffer, opts = {}) {
  const defaultOpts = {
    folder: process.env.CLOUDINARY_FOLDER || 'mnb',
    resource_type: 'auto',
    use_filename: true,       // use our own name
...
const createSong = async (req, res, next) => {
  try {
    ...
    // ✅ Upload audio with custom public_id based on slug
    const audioUp = await uploadBufferToCloudinary(audioBuffer, {
      resource_type: "auto",
      public_id: `songs/${slug}`, // This controls the filename
      overwrite: true
    });

    // ✅ Upload cover image (optional) with slug-based name
    let imageUp;
    if (imageFile) {
      imageUp = await uploadBufferToCloudinary(imageFile.buffer, {
        resource_type: "image",
...
const updateSong = async (req, res) => {
  try {
    ...
    if (audioFile) {
      ...
      const audioUp = await uploadBufferToCloudinary(audioBuffer, {
        resource_type: "auto",
        public_id: `songs/${song.slug}`, // use song.slug, not slug
        overwrite: true
      });
    ...
    if (imageFile) {
      const imageUp = await uploadBufferToCloudinary(imageFile.buffer, {
        resource_type: 'image',
        public_id: `covers/${song.slug}`, // keep consistent naming
        overwrite: true
      });
